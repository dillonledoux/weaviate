...
// asyncReplicationConfigFromModel clamps sub-floor frequency /
// frequencyWhilePropagating values up to the floor (logging a Warn) and
...
func TestAsyncReplicationConfigFromModelFrequencyFloor(t *testing.T) {
	// frequency floor is 5s, frequencyWhilePropagating floor is 1s — pick
	// boundary inputs relative to each field's own minimum.
	...
	t.Run("frequency_below_min_clamped_and_warned", func(t *testing.T) {
		...
		assert.Equal(t, minFrequency, *cfg.classOverrides.frequency,
			"sub-floor frequency must be clamped to minFrequency")
	...
	t.Run("frequency_at_min_accepted_no_warn", func(t *testing.T) {
		...
		require.NotNil(t, cfg.classOverrides.frequency)
		assert.Equal(t, minFrequency, *cfg.classOverrides.frequency)
		assert.Empty(t, hook.Entries, "no warning expected at floor")
	})
	...
	t.Run("frequency_above_min_accepted_no_warn", func(t *testing.T) {
		...
		assert.Empty(t, hook.Entries, "no warning expected above floor")
	})
	...
	t.Run("frequencyWhilePropagating_below_min_clamped_and_warned", func(t *testing.T) {
		...
		assert.Equal(t, minFrequencyWhilePropagating, *cfg.classOverrides.frequencyWhilePropagating,
			"sub-floor frequencyWhilePropagating must be clamped to minFrequencyWhilePropagating")
		require.Len(t, hook.Entries, 1, "exactly one Warn entry expected")
		assert.Equal(t, logrus.WarnLevel, hook.LastEntry().Level)
	...
	//   - zero: treated as below-floor, clamped up to the floor with a Warn,
	...
	//     to a negative time.Duration and silently clamping to the floor.
	...
	// to the floor. Reject them explicitly so callers don't end up with the
...
func TestEffectiveThreeTierPrecedence(t *testing.T) {
	...
	t.Run("global_config_subfloor_clamps_to_floor", func(t *testing.T) {
		// A positive but sub-floor global DynamicValue must clamp up to the floor
		// rather than being silently dropped — otherwise an operator who tried to
		// dial frequency down past the floor would get the previous (class or
		// default) value with no signal that their override was ineffective.
		cfg := AsyncReplicationConfig{
			frequency: codeDefault,
			classOverrides: asyncReplicationClassOverrides{
				frequency: durationPtr(classOverride),
		...
		assert.Equal(t, minFrequency, result.frequency,
			"sub-floor global DynamicValue must clamp to minFrequency, not be ignored")
	})
	...
	t.Run("global_config_frequencyWhilePropagating_subfloor_clamps_to_its_own_floor", func(t *testing.T) {
		...
		// distinct minFrequencyWhilePropagating floor.
		...
		assert.Equal(t, minFrequencyWhilePropagating, result.frequencyWhilePropagating,
			"sub-floor global DynamicValue must clamp to minFrequencyWhilePropagating, not minFrequency, and not be ignored")
	})
}
