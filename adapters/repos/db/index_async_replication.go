...
//
// Frequency floors (minFrequency, minFrequencyWhilePropagating) are enforced
// by clamping sub-floor values up to the floor and emitting a Warn log entry
// via the supplied logger. This is intentionally lenient so that collections
// created before a floor was raised continue to load and update; the runtime
// defensive clamp in Effective() would otherwise be the only safety net and
// would silence the legacy values without operator visibility.
