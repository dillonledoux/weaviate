...
USAGE
  release.sh [--journal] [command] [args...]
...
cmd_prepare() {
  ...
  # Bump .info.version in schema.json and verify the on-disk value before
  # recording the step.
  local CURRENT; CURRENT="$(git show "origin/${STABLE_BRANCH}:${SPEC}" | jq -r '.info.version')"
  if [[ "$CURRENT" != "$VERSION" ]]; then
    echo ">>> Bumping schema.json: ${CURRENT} → ${VERSION}"
    local tmp; tmp=$(mktemp)
    jq --arg v "$VERSION" '.info.version = $v' "$SPEC" > "$tmp"
    mv "$tmp" "$SPEC"
    ...
    echo ">>> schema.json already at ${VERSION}"
    state_ensure schema_bump from="$VERSION" to="$VERSION"
  fi
  ...
  if git rev-parse "v${VERSION}" &>/dev/null 2>&1; then
    echo ">>> Tag v${VERSION} already exists locally — skipping prepare_release.sh"
    state_ensure prepare_release_sh tag=preexisting
  else
    echo ">>> Running prepare_release.sh"
    tools/prepare_release.sh "prepare release v${VERSION}"
    echo ""
...
_NEWARGS=()
for _a in "$@"; do
  [[ "$_a" == "--journal" ]] && JOURNAL_ENABLED=1 || _NEWARGS+=("$_a")
done
